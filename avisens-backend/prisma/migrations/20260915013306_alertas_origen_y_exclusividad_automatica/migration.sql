-- CreateEnum
CREATE TYPE "OrigenAlerta" AS ENUM ('automatica', 'manual', 'desconocido');

-- AlterTable
ALTER TABLE "alertas" ADD COLUMN     "origen" "OrigenAlerta" NOT NULL DEFAULT 'desconocido';
...
-- umbrales_un_vigente_por_semana: indice unico parcial via SQL manual).
CREATE UNIQUE INDEX "alertas_una_automatica_activa_por_sensor"
  ON "alertas" ("sensor_id")
  WHERE "origen" = 'automatica'::"OrigenAlerta" AND "estado" IN ('abierta', 'en_proceso');
