...
describe('AnalisisVisionService', () => {
  let service: AnalisisVisionService;
  const prisma = {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AnalisisVisionService,
        { provide: PrismaService, useValue: prisma },
      ],
    }).compile();
    service = module.get(AnalisisVisionService);
  ...
  it('registra un análisis con relaciones consistentes', async () => {
    ...
    prisma.analisisVision.create.mockResolvedValue({ id: 1 });
    await expect(
      service.crear(
        {
          galpon_id: 2,
  ...
  it('aplica el alcance del propietario al listado', async () => {
    ...
    const llamadas = prisma.analisisVision.findMany.mock
