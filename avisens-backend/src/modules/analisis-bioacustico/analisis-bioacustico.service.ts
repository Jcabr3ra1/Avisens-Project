...
@Injectable()
export class AnalisisBioacusticoService {
  ...
  async crear(dto: CreateAnalisisBioacusticoDto, solicitante: Solicitante) {
    ...
    return this.prisma.analisisBioacustico.create({
      data: {
        ...
        interpretacion: dto.interpretacion,
      },
      select: SELECT,
    });
  }
  ...
  async listar(
    ...
    const [data, total] = await this.prisma.$transaction([
      this.prisma.analisisBioacustico.findMany({
        where,
        select: SELECT,
      ...
      this.prisma.analisisBioacustico.count({ where }),
    ]);
    return paginate(data, total, page, limit);
  }

  async obtener(id: number, solicitante: Solicitante) {
    const analisis = await this.prisma.analisisBioacustico.findUnique({
      where: { id },
      select: SELECT,
    });
    if (!analisis)
      throw new NotFoundException('Análisis bioacústico no encontrado');
    await verificarAccesoGalpon(
      this.prisma,
      analisis.galpon_id,
      solicitante,
  ...
  async actualizar(
    id: number,
    dto: UpdateAnalisisBioacusticoDto,
    solicitante: Solicitante,
  ) {
    ...
    return this.prisma.analisisBioacustico.update({
      where: { id },
      data: {
        galpon_id: dto.galpon_id,
        lote_id: dto.lote_id,
  ...
  async eliminar(id: number, solicitante: Solicitante) {
    ...
    await this.prisma.analisisBioacustico.delete({ where: { id } });
