...
const initialMsgs: Record<number, Msg[]> = {
  1: [
    { from: 'them', text: 'Buen día. La zona 2 sigue con temperatura alta, ya bajé la velocidad del extractor 3.', time: '08:42' },
    { from: 'them', text: 'Voy a revisar el sensor por si está sucio.', time: '08:43' },
    { from: 'me',   text: '¿Necesitas que abra la ventilación por el lado este?', time: '08:45' },
    { from: 'them', text: 'Sí, déjala en manual al 70% un par de horas. Te aviso cuando estabilice.', time: '08:46' },
  ],
  2: [{ from: 'them', text: 'Mortalidad sigue dentro del rango. Mañana hago revisión visual del lote.', time: 'Ayer' }],
  4: [{ from: 'them', text: 'Galpón 2 al día. Mando reporte en la tarde.', time: '09:12' }],
}
...
const ChatTeam = ({ onJump }: Props) => {
  ...
  const send = () => {
    ...
    setTimeout(() => {
      setMsgs(m => ({
        ...m, [activeId]: [...(m[activeId] || []), { from: 'them', text: 'Recibido, lo reviso ya.', time }],
      }))
    }, 1600)
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      ...
      <div style={{ padding: '14px 18px 8px' }}>
        <div style={{ fontSize: 10, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 8 }}>
          Conversación con
        </div>
        <div style={{
          background: '#ecfdf5', borderRadius: 14, padding: '12px 14px',
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text3)' }}>{active.role}</div>
          </div>
          <button style={panelIconBtn(true)}><IcMic size={14} /></button>
          <button style={panelIconBtn()}><IcChevronDown size={14} /></button>
        </div>
      </div>

      {/* Lista de equipo */}
      <div style={{ padding: '6px 18px 4px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontSize: 11, color: 'var(--text3)' }}>Equipo</div>
        <button onClick={onJump} style={{ ...panelTextBtn }}>+ Iniciar grupo</button>
      </div>
      <div className="panel-scroll" style={{ padding: '4px 18px 8px', display: 'flex', gap: 10, overflowX: 'auto' }}>
        {initialOperators.map(op => (
          <button key={op.id} onClick={() => setActive(op.id)} style={{
            border: 0, background: 'transparent', display: 'flex', flexDirection: 'column',
            alignItems: 'center', gap: 4, padding: 0, color: 'inherit', minWidth: 50, cursor: 'pointer',
          }}>
            ...
            <div style={{
              fontSize: 10, color: activeId === op.id ? 'var(--green-d)' : 'var(--text3)',
              fontWeight: activeId === op.id ? 600 : 400,
      ...
      <div ref={listRef} className="panel-scroll" style={{
        flex: 1, minHeight: 0, overflowY: 'auto', padding: '12px 18px',
        display: 'flex', flexDirection: 'column', gap: 8,
        background: 'var(--bg-tint)',
        borderTop: '1px solid var(--border)',
        borderBottom: '1px solid var(--border)',
      }}>
        <div style={{ textAlign: 'center', fontSize: 10, color: 'var(--text4)', margin: '6px 0' }}>HOY · 8 DE MAYO</div>
      ...
      <div style={{ padding: '10px 14px 16px' }}>
        <div style={{
          ...
          background: '#ffffff', border: '1.5px solid var(--border)', borderRadius: 14, padding: 5,
        }}>
          ...
          <button
            onClick={send}
            style={{
              background: draft.trim() ? 'var(--green-d)' : 'var(--bg-tint)',
              color: draft.trim() ? '#ffffff' : 'var(--text4)',
